...
export class CollectesComponent implements OnInit {
  ...
  private loadDropdowns(): void {
    ...
    this.collecteApi.getUsersByRole('RESPONSABLE_LOGISTIQUE', 'DISPONIBLE').subscribe({
      next: (res) => (this.responsablesAffectation = res.users ?? []),
      error: () => {},
    });

    this.collecteApi.getUsersByRole('OUVRIER', 'DISPONIBLE').subscribe({
  ...
  openCreateForm(): void {
    ...
    this.collecteForm.reset({ statut: 'PLANIFIEE' });
    this.showForm = true;
